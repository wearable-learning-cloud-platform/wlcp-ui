		...
		for(var key in loadData.globalVariables) {
			for(var i = 0; i < iconTabs.length; i++) {
				if(key == iconTabs[i].scope) {
					for(var n = 0; n < iconTabs[i].navigationContainerPages.length; n++) {
						if(iconTabs[i].navigationContainerPages[n].title == this.getNavigationContainerPage().title) {
                            for(var k = 0; k < loadData.globalVariables[key].variableModifiers.length; k++) {
								iconTabs[i].navigationContainerPages[n].items.push({variableName : loadData.globalVariables[key].variableModifiers[k].variableName, operator : loadData.globalVariables[key].variableModifiers[k].operator, value : loadData.globalVariables[key].variableModifiers[k].value});
							}
						}
					}
				}
	...
	getSaveData() {
		...
		for(var i = 0; i < iconTabs.length; i++) {
			for(var n = 0; n < iconTabs[i].navigationContainerPages.length; n++) {
				if(iconTabs[i].navigationContainerPages[n].title == this.getNavigationContainerPage().title) {
					if(iconTabs[i].navigationContainerPages[n].items.length > 0) {
                        var variableModifiers = [];
                        for(var k = 0; k < iconTabs[i].navigationContainerPages[n].items.length; k++) {
                            variableModifiers.push({variableName : iconTabs[i].navigationContainerPages[n].items[k].variableName, operator : iconTabs[i].navigationContainerPages[n].items[k].operator, value : iconTabs[i].navigationContainerPages[n].items[k].value});
                        }
